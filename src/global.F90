...
module omp_parallel
   ...
   integer              :: ompThreads, ompSplit
   integer, allocatable :: limit_lower(:)
   integer, allocatable :: limit_upper(:)
...
module time
   ...
   real(kind=4)     :: startTimeInit, startTimeCalc, endTimeInit, endTimeCalc
...
module output_data
   ...
   logical            :: outputToSingleFile, outputEnergyConservation, outputTimesteps, outputSilentMode
...
module bodies
   ...
   integer                       :: nrOfBodies, maxNrOfBodies, bodiesRemoved
   ...
   double precision, allocatable :: body_axp(:,:), body_ayp(:,:)               ! body accelerations on previous timesteps
...
module energy
   ...
   subroutine calculateEnergies
      ...
      integer          :: i, j
      double precision :: localE_kin, localE_pot
      
      localE_kin=0.0d0
      localE_pot=0.0d0
      
      do i = 1, nrOfBodies
         localE_kin = localE_kin + 0.5d0 * body_mass(i) * (body_vx(i)*body_vx(i) + body_vy(i)*body_vy(i))
         do j = i+1, nrOfBodies
            localE_pot = localE_pot - gravitationalConstant * (body_mass(i)*body_mass(j)) / & 
...
module barnes_hut
   ...
   integer          :: nodes
   integer          :: idCounter
   double precision :: minimumCollisionTimeSqr
   ...
   !------------------------------      
   recursive subroutine calculateForce(bht, bid, f)
      implicit none
      ...
      if (bht%nrOfBodies .eq. 1) then
         if (bht%body_id .ne. bid) then
            call calculateForceBetweenObjects(bid, body_x(bht%body_id), body_y(bht%body_id), &
                                                   body_mass(bht%body_id), .true., f, bid2=bht%body_id)
         end if
      else
         ...
         if ((bht%quad%lengthSqr / rsqr) < thetaSqr) then
            call calculateForceBetweenObjects(bid, bht%com_x, bht%com_y, bht%mass, .false., f, r=r, rsqr=rsqr)
         else
            if (associated(bht%NW)) then
               call calculateForce(bht%NW, bid, fr); f = f + fr
            end if
            if (associated(bht%NE)) then
               call calculateForce(bht%NE, bid, fr); f = f + fr
            end if
            if (associated(bht%SW)) then
               call calculateForce(bht%SW, bid, fr); f = f + fr
            end if
            if (associated(bht%SE)) then
               call calculateForce(bht%SE, bid, fr); f = f + fr
            end if
         end if
      end if
   ...
   !------------------------------      
   subroutine calculateForceBetweenObjects(bid, bx, by, bm, d, f, r, rsqr, bid2)
      implicit none
      logical         , intent(in)           :: d
      integer         , intent(in)           :: bid
      ...
      double precision                       :: v(2), colTimeSqr
      double precision                       :: f1
      ...
      if (d) then
         if (do_dtCalculation) then
            ...
            colTimeSqr = r3 / (v(1)*v(1) + v(2)*v(2))
            
            if (colTimeSqr .lt. minimumCollisionTimeSqr) then
               !$omp critical
               minimumCollisionTimeSqr = colTimeSqr
               !$omp end critical
            end if
         end if
